...
Avec un SVM à noyau linéaire entraîné sur les classes 1 et 2 du jeu de données Iris, le meilleur paramètre trouvé est C ≈ 8.31.
Le modèle atteint un score d’entraînement de 73% et un score de test de 72%.
...
Le meilleur compromis est obtenu avec C ≈ $1.0 \times 10^{-3}$, qui donne un taux de précision de 91% sur le test, bien supérieur au hasard (62%).
...
Sans ajout de variables de nuisance, le SVM linéaire obtient un taux de précision parfait à l’entraînement (1.0) et une très bonne performance en test : 0.911 .
En revanche, lorsque l’on ajoute 300 variables bruitées, la performance en test chute fortement (0.516), tandis que l’entraînement reste à 1.0.
