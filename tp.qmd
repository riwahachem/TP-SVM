...
## Mise en oeuvre de la base de données Iris

Cette section est destinée à l’étude de la base de données Iris, sur laquelle nous appliquerons un classifieur à vecteurs de support. Afin d’évaluer correctement les performances du modèle, l’ensemble des données est divisé en deux sous-ensembles : un échantillon d’apprentissage représentant 75 % des observations et un échantillon de test correspondant aux 25 % restants. Dans un premier temps, nous considérons un noyau linéaire, puis nous étudions un noyau polynomial, afin de comparer leurs performances respectives et d’analyser l’impact du choix du noyau sur la frontière de décision et la qualité de la classification.
...
Avec la réduction de dimension par PCA, le modèle conserve un taux de précision parfait sur l’entraînement mais la performance en test reste faible (≈ {{ score_test_pca:.3f }} pour 200 composantes). Cela s’explique par le fait qu’un trop grand nombre de composantes conserve encore beaucoup de bruit.

### Question 7

Dans le script fourni, la normalisation est effectuée avant la séparation entre apprentissage et test, comme l’illustre l’extrait de code de la Figure 3.

![](images/code.png){fig-align="center" width=90%}

<div style="text-align: center;">
*Figure 3 – Extrait du code de normalisation avant séparation train/test*
</div>
