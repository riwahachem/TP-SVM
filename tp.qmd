...
Le modèle atteint un score d’entraînement de 0.747 et un score de test de 0.680.
...
plt.draw()
```
Sur le dataset Iris, nous avons entraîné un SVM à noyau linéaire et un SVM à noyau polynomial.
Avec le noyau linéaire, le modèle atteint de bons scores en entraînement comme en test, ce qui montre que les deux classes sont presque séparables par une frontière droite.
Avec le noyau polynomial, les résultats restent similaires mais la frontière devient plus complexe et peut conduire à un léger sur-apprentissage selon le degré choisi.
Cela illustre que, dans un espace déjà quasi linéairement séparable, un noyau polynomial n’apporte pas de gain significatif.

## SVM GUI

### Question 3

Dans le script `svm_gui.py`, le paramètre de régularisation C contrôle la complexité du classifieur dans la mesure où il détermine le coût d’une mauvaise classification : plus C est grand, plus la règle obtenue est complexe. Ce paramètre peut être ajusté directement depuis l’interface graphique.
Lorsque l’on sélectionne un noyau linéaire et que l’on diminue la valeur de C, on constate que la frontière de décision devient moins stricte. Concrètement, certains points, bien que séparables, se retrouvent du mauvais côté de l’hyperplan. Ainsi, le modèle accepte davantage d’erreurs d’entraînement afin de maximiser la marge séparatrice.
Dans un contexte de classes déséquilibrées (par exemple 90 % d’exemples d’une classe contre 10 % pour l’autre), une valeur faible de C amplifie ce déséquilibre, c'est à dire que le classifieur privilégie la classe majoritaire et néglige les erreurs sur la classe minoritaire.
...
print(f"Accuracy finale sur le test : {clf.score(X_test, y_test):.3f}")
...
En faisant varier le paramètre C, on observe que les performances augmentent jusqu’à un optimum puis se stabilisent.
Le meilleur résultat est obtenu pour C ≈ $1.0 \times 10^{-3}$ avec une accuracy d’environ 0.916 sur l’échantillon de test.
Lorsque C est trop petit, le modèle sous-apprend (fort biais), et lorsque C est trop grand, il risque de sur-apprendre.
La visualisation des coefficients montre quelles zones du visage contribuent le plus à la décision.
...
Sans ajout de variables de nuisance, le SVM linéaire obtient une accuracy parfaite à l’entraînement (1.0) et une très bonne performance en test :`r {score_test:.3f}`.
...
Avec la réduction de dimension par PCA, le modèle conserve une accuracy parfaite sur l’entraînement mais la performance en test reste faible (≈ `r {score_test_pca:.3f}` pour 200 composantes). Cela s’explique par le fait qu’un trop grand nombre de composantes conserve encore beaucoup de bruit.
